...
/* واجهةُ المتون الأولى لا تُخرج إلا عشرة كتب. وما زدناه عليها — الدارمي
   وبلوغ المرام ورياض الصالحين ومشكاة المصابيح والأدب المفرد والشمائل —
   متنُه مشحونٌ عندنا بابًا بابًا بالشكل نفسه، فتقرؤه الدوالُّ التالية بلا
   أن تعرف من أين جاء. وأسماءُ هذه الكتب في ملفٍّ يكتبه سكربتُ الجلب، فلا
   تُنسخ هنا ولا تفترق عنه. */
let _lb=null;
const localBook=async(b)=>{
  if(!_lb) _lb=api.local("txt/books.json").then(a=>new Set(a)).catch(()=>new Set());
  return (await _lb).has(b);
};
const api={
  ...
  hadith:async(b,n)=>{
    if(!await localBook(b))
      return grab("fawazahmed0/hadith-api",REF.hadith,`editions/ara-${b}/${n}.json`);
    /* بابُ الحديث في فهرسنا، ومتنُه في ملفّ بابه */
    const row=(await loadIdx(b)).find(r=>r[0]===n);
    if(!row) return {hadiths:[]};
    const d=await api.local(`txt/${b}/${row[1]}.json`);
    const h=(d.hadiths||[]).find(x=>+x.hadithnumber===n);
    return {hadiths:h?[h]:[]};
  },
  section:async(b,n)=>{
    if(await localBook(b)) return api.local(`txt/${b}/${n}.json`);
    return grab("fawazahmed0/hadith-api",REF.hadith,`editions/ara-${b}/sections/${n}.json`);
  },
};
...
const BOOKNOTE={bukhari:'أصحّ الكتب بعد كتاب الله، جمعه البخاري في نحو ستة عشر عامًا.',
 ...
 darimi:'تاسع الكتب التسعة، ومن أقدمها تصنيفًا، رتّبه على الأبواب كالسنن.',
 riyad:'منتخَبٌ في الآداب والأخلاق، أشهر كتب الترغيب وأكثرها قراءةً.',
 mishkat:'جمع أحاديث المصابيح للبغوي وزاد عليها، وبيّن موضع كل حديث من الكتب.',
 bulugh:'أحاديث الأحكام مختصرة، يذكر مع كل حديث من أخرجه من الأئمة.',
 adab:'أفرده البخاري لأحاديث الأخلاق والبرّ وصلة الرحم.',
 shamail:'في صفته ﷺ: خَلقِه وخُلقِه وهديه في يومه وليلته.',
...
const SRCBOOKS=[
 ["كتب الحديث",[
  ...
  ["سنن الدارمي","للإمام عبد الله بن عبد الرحمن الدارمي. تاسع الكتب التسعة، ومن أقدمها تصنيفًا، مرتّب على الأبواب كالسنن. ٣٬٤٠٦ أحاديث."],
  ["رياض الصالحين","للإمام النووي. منتخَب في الآداب والأخلاق، أشهر كتب الترغيب وأكثرها قراءةً. ١٬٨٩٦ حديثًا."],
  ["مشكاة المصابيح","للخطيب التبريزي. جمع أحاديث المصابيح للبغوي وزاد عليها، وبيّن موضع كل حديث من الكتب. ٤٬٤٢٨ حديثًا."],
  ["بلوغ المرام","للحافظ ابن حجر العسقلاني. أحاديث الأحكام مختصرة، يذكر مع كل حديث من أخرجه من الأئمة. ١٬٧٦٧ حديثًا."],
  ["الأدب المفرد","أفرده الإمام البخاري لأحاديث الأخلاق والبرّ وصلة الرحم. ١٬٣٢٦ حديثًا."],
  ["الشمائل المحمدية","للإمام الترمذي، في صفته ﷺ: خَلقِه وخُلقِه وهديه في يومه وليلته. ٤٠٢ حديثًا."],
 ...
 ["كتب شرح الحديث",[
  ...
  ["سبل السلام للصنعاني","شرح بلوغ المرام لابن حجر، يُعنى بأحاديث الأحكام وما استُنبط منها."],
  ["مرقاة المفاتيح لملّا علي القاري","شرح مشكاة المصابيح، وهو أوسع شروحها."],
  ["جمع الوسائل في شرح الشمائل لملّا علي القاري","شرح الشمائل المحمدية للترمذي."],
  ["دليل الفالحين لابن علّان الصدّيقي","شرح رياض الصالحين للنووي."]]],
 ["كتب السيرة",[
 ...
 ["كتب أخرى",[
  ...
  ["التخريج: موضع الحديث في بقية الكتب","٤٣٬٦٢١ وصلًا بين مواضع الحديث الواحد، مستخرجة بمطابقة اللفظ لا بالظنّ: يُشترط أن يشترك المتنان في أكثر الأقصر منهما لفظًا بلفظ، وأن يكون بينهما لفظ متّصل لا يقلّ عن ست كلمات، وأن يكون في ذلك اللفظ أربع كلمات مميّزة على الأقل — فلا يُوصل حديثان لاشتراكهما في أسماء رجال أو ألفاظ رواية. والمطابقة متحفّظة، فقد تفوتها بعض الطرق."],
  ["نصّ القرآن الكريم","بالرسم الإملائي المعياري."]]],
 ["ما نقصنا فنقوله",[
  ["ثغرة في متون ٤٠٨ أحاديث","من ٤٩٬٧٣٧ حديثًا (٠٫٨٪) خلا مصدرُ متونها من نصّها، أكثرها في مقدّمتي صحيح مسلم وسنن النسائي. أرقامها وأبوابها وأحكامها ثابتة، ويُقال في صفحاتها صراحةً إنّ المتن لم يرد، ولا يُملأ من عندنا. وقد فُحصت نسخة عربية أخرى من المصدر نفسه فوُجدت الثغرة فيها كما هي."],
  ["أحكام المحدّثين لا تشمل كل الكتب","الأحكام المنسوبة إلى قائليها موجودة لأحاديث الكتب الستة والموطأ. وأمّا سنن الدارمي ورياض الصالحين ومشكاة المصابيح وبلوغ المرام والأدب المفرد والشمائل والأربعينات، فلم نجد لها مصدرًا يذكر الحكم ومن قاله معًا. وحكمٌ بلا قائل لا نعرضه، وقائلٌ نخمّنه كذب. فبدل ذلك نصل الحديث بموضعه من الكتاب الذي أخرجه — وهناك حكمه بأسماء من حكموا عليه. ننقل الموضع، لا الحكم."],
  ["السند حين لا يسوقه الكتاب","سنن الدارمي والأدب المفرد والشمائل تسوق أسانيدها كاملة في نصّها، فاستُخرجت منه: ٤٬٩٦٣ سلسلة. ويُقال في صفحاتها صراحةً إنّها مستخرَجة آليًّا وقد تنقص. والاسم لا يُربط بترجمة راوٍ إلا إن انفرد بذلك الاسم راوٍ واحد؛ وإن تعدّد المسمّون به تُرك اسمًا بلا رابط، فنحن لا نجزم بهوية ملتبسة. وبلوغ المرام ورياض الصالحين ومشكاة المصابيح منتخَبات لا تسوق أسانيد أصلًا، فسندها يُطلب في الكتاب الذي أخرج الحديث، ونحن ندلّ عليه."],
  ["أربعة كتب بلا شرح مشحون","سنن الدارمي والأدب المفرد والأربعون القدسية وأربعون الشاه ولي الله: لم نجد لها شرحًا مهيكلًا في المدوّنات المنشورة، فيُقال ذلك في تبويب الشرح ولا يُوضع مكانه شيء."],
  ["مسند أحمد غير مشحون","المصدر المهيكل الذي بين أيدينا لا يحمل منه إلا ١٬٣٧٤ حديثًا من نحو سبعة وعشرين ألفًا (ثمانية فصول من واحد وثلاثين، والنقص عند المصدر نفسه). وعرضُ خمسة في المئة باسم «مسند أحمد» يوهم أنّه المسند، فتُرك حتى يوجد مصدر تامّ."],
 ]],
];
